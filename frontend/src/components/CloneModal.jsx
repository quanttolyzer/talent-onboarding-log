...
export default function CloneModal({ rows, onClose, onSaved }) {
  // Only editable fields — locked fields (position, dept, managers, country) keep original
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box">
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
          <div>
            ...
            <p style={{ fontSize:'0.82rem', color:'var(--text-2)', marginTop:'4px' }}>
              Locked fields (position, department, managers, country) are inherited from original rows.
              Fill only the fields you want to override — blank = keep original.
            </p>
          </div>
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid var(--border)', borderRadius:'var(--radius-sm)', padding:'12px 16px', marginBottom:'20px', fontSize:'0.82rem' }}>
          <div style={{ color:'var(--text-3)', fontWeight:600, marginBottom:'8px', textTransform:'uppercase', letterSpacing:'0.04em' }}>Locked (from original)</div>
          {rows.length === 1 ? (
            ...
            <span style={{ color:'var(--text-2)' }}>Multiple rows selected — each clone inherits its own original values</span>
          )}
        </div>
        ...
        <div className="form-grid form-grid-2" style={{ marginBottom:'18px' }}>
          ...
          <div className="form-group">
            ...
            <select value={overrides.sub_action} onChange={e => set('sub_action', e.target.value)} disabled={!overrides.action}>
              <option value="">Keep original</option>
          ...
          <div className="form-group" style={{ gridColumn:'span 2' }}>
            ...
            <input value={overrides.remarks} onChange={e => set('remarks', e.target.value)} placeholder="Leave blank to keep original" />
          </div>
        </div>
